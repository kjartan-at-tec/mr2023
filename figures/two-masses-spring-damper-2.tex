...
\tikzstyle{spring}=[decorate,decoration={zigzag,pre length=0.2cm,post length=0.2cm,segment length=6}]
  ...
  \coordinate (w1s) at ($(wall.north) + (0,0.5cm)$);
  \coordinate (m12s) at ($(M1.west) + (0,0.5cm)$);
  \coordinate (wd) at ($(wall.north) + (0,-0.5cm)$);
  \coordinate (m12d) at ($(M1.west) + (0,-0.5cm)$);

  \draw[spring] (w1s)  --  node[above] {$k_1$} (m12s);
  \draw[damper] (wd)  --  node[below, pos=0.3] {$b$} (m12d);
