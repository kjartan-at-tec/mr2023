...
L2A,% Linealización local, modelo de coche
L3,% Linealización local ejemplo numerico
%L31,% Linealización local ejemplo numerico
%G0,% ODE segunda orden, Laplace, trf
LA0,% Laplace
LA1A,% Laplace
LA2A,% Laplace
...
\begin{frame}[label=C0]{Modelación en el contexto de automatización}
  \footnotesize
  \begin{center}
    \begin{tikzpicture}[scale=0.8, transform shape,
      ...
      \begin{scope}[yshift=-3cm,]
        \node[block] (cc) {Controlador};
        \node[sumnode, left of=cc] (sum) {$\Sigma$};
        \node[block, right of=cc, node distance=3cm,] (actuator) {Actuador};
        \node[block, right of=actuator, node distance=3cm,] (plant) {Coche};
...
\begin{frame}[label=I1]{Modelo de masa puntual}
...
\begin{frame}[label=L2A]{Linealización}
  ...
  \begin{columns}
    \begin{column}{0.6\columnwidth}
      \begin{description}
        \item[Define el punto de operación \(v_0\) y la variable de desviación \(y\)]
          \[ v(t) = v_0 + y(t) \quad \Leftrightarrow \quad y(t) = v(t) - v_0\]
          \pause
        \item[Aproxima la función usando la serie de Taylor]
          \begin{align*}
      ...
      \end{align*}
          \pause
      \item[Substitye la aproximación en la ecuación diferencial]
      \begin{align*}
...
\begin{description}
          \pause
  \item[Ecuación diferencial linealizado]
    \[m\dot{y} = -2kv_0y + u\]
  \end{description}
\end{column} 
\end{columns}
...
\end{frame}
  ...
  \begin{frame}[label=LA0]{Transformada de Laplace}
  ...
  \begin{frame}[label=LA1A]{Transformada de Laplace}
...
\begin{center}
  ...
  \begin{frame}[label=M0]{Modelación de un sistema mecánico}
    \small
    ...
    \begin{center}
      ...
      \end{tikzpicture}\\
      Un motor impulsa una carga a través de un eje elástico. Encuentra las funciónes de transferencia $G(s)$ y $H(s)$.
    \end{center}

\end{frame}


  \begin{frame}[label=M1]{Modelación de un sistema mecánico}
    \small
    ...
    \end{center}
      ...
      Usando la segunda ley de Newton para sistemas rotacionales
      ...
      \item[Diagrama de cuerpo libre]
          \begin{align*}
            \text{Cuerpo 1:}\qquad & J_1 \ddot{\theta}_1 = T_i - k (\theta_1 - \theta_2) - b(\dot{\theta}_1 - \dot{\theta}_2)\\
            \text{Cuerpo 2}: \qquad & J_2 \ddot{\theta}_2 = k (\theta_1 - \theta_2) - b(\dot{\theta}_1 - \dot{\theta}_2) - T_l
          \end{align*}
        \pause
       \item[Aplica Laplace]
      \begin{align*}
