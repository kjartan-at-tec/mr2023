...
\tikzstyle{spring}=[decorate,decoration={zigzag,pre length=0.2cm,post length=0.2cm,segment length=6}]
  ...
  \node (M1) [thick, draw, minimum width=\masswidth cm, minimum height=\massheight cm] {$m$};

  \node (ground) [ground,anchor=north west, xshift = -2cm, yshift=-0.25cm,minimum width=4cm] at (M1.south) {};
  \draw (ground.north east) -- (ground.north west);

  \node (wall) [ground, rotate=-90, minimum width=3cm,yshift=-3cm] {};
  \draw (wall.north east) -- (wall.north west);

  \node (wall2) [ground, rotate=-90, minimum width=3cm,yshift=3cm] {};
  \draw (wall2.south east) -- (wall2.south west);

  \draw[spring] (wall.north)  --  node[below] {} (M1.west);
  \draw[spring] (wall2.south)  --  node[below] {} (M1.east);
  ...
  \draw[->, thick] (M1.north east) ++(0,-0.2cm) -- ++(1cm, 0) node[very near end, above] {$u(t)$};

  \draw[dashed] (M1.north west) -- ++(0, 14mm);

  \draw[->] (M1.north west) ++ (0, 8mm) -- ++(1cm, 0) node[right] {$z$};
